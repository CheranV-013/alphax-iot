...
Browser GPS is opt-in. The dashboard shows `ALLOW LIVE LOCATION`; only after the visitor clicks it does the browser start `watchPosition()` with high accuracy. GPS coordinates and accuracy are then sent through `/api/visitor/heartbeat`. Location priority is GPS, then cached approximate IP geolocation, then `UNKNOWN`. Denied/unavailable GPS never stops visitor heartbeats.
