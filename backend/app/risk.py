...
def assess(tx, user, device, ml):
    ...
    features={"amount":tx.amount,"velocity":tx.transaction_velocity,"device_change":int(not known_device),"location_change":location_change,"ip_risk":ip_score,"spend_deviation":(tx.amount-user.get("baseline",100))/max(user.get("baseline",100),1),"hour":tx.timestamp.hour}
    fraud, anomaly=ml.infer(features)
    parts={"Fraud probability":fraud,"Anomaly detection":anomaly,"Unusual behaviour":behaviour,"IP reputation":ip_score,"New device":device_score,"Location anomaly":loc_score,"IoT tamper":tamper}
    score=100*(.25*fraud+.20*anomaly+.15*behaviour+.10*ip_score+.10*device_score+.10*loc_score+.10*tamper)
